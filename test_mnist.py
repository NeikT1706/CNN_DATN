...
model2 = torch.load("model_mnist_svd.pth")
model.eval()
model2.eval()
...
# model = torch.load("qat_model.pth")
...
mnist_svd_time = time.time() - start_time2
# Tính độ chính xác
accuracy = 100 * correct / total
print(f'Accuracy of the model on MNIST with SVD test images: {accuracy:.2f}%')
print(f'Time: {mnist_svd_time} second')

# Tên các biến và giá trị tương ứng
categories = ['CNN', 'CNN with SVD']
values = [mnist_time, mnist_svd_time]
...
plt.xlabel('')
plt.ylabel('Time (s)')
plt.title('Time Comparison for CNN and CNN with SVD on MNIST Dataset')
plt.grid(axis='y', linestyle='--', alpha=0.7)  # Lưới chỉ trên trục y

# Hiển thị và lưu biểu đồ
plt.savefig('TimeComparison_MNIST.svg', format='svg')
plt.show()